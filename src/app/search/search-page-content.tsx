...
type SearchPageContentProps = {
  ...
  backstagePosts: BackstagePostSummary[];
  services: Service[];
};

export function SearchPageContent({ projects, backstagePosts, services }: SearchPageContentProps) {
  ...
  const results = useMemo(() => {
    ...
    ].filter((item) => (item.title + item.excerpt + (item.tags ?? []).join(' ')).toLowerCase().includes(normalized));
  }, [backstagePosts, normalized, projects, query, services]);
